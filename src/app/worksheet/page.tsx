      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {ALL_LEVELS.map((lv) => (
                <button
                  key={lv}
                  onClick={() => { setLevel(lv); setSelectedTypes([]); }}
                ...
                >
                  等级 {lv}
                  <span className="block text-xs opacity-75">{LEVEL_NAMES[lv]}</span>
                </button>
              ))}
            </div>
            <div className="mt-3">
          ...
          <div className="card p-5">
            <label className="block font-bold text-gray-700 mb-3">
              🔢 题目数量：
              <span className="text-blue-500 text-xl ml-2">{questionCount}</span> 题
            </label>
            <input
              type="range"
              min={5} max={50} step={5}
          ...
          <div className="card p-5">
            <div className="flex items-center justify-between mb-3">
              <label className="font-bold text-gray-700">✏️ 选择题型</label>
            ...
            <p className="text-xs text-gray-400 mt-2">
              {selectedTypes.length === 0 ? '未选择时默认包含全部题型' : `已选择 ${selectedTypes.length} 种题型`}
            </p>
          </div>
          ...
          >
            {isDownloading ? '⏳ 正在生成...' : '⬇️ 下载练习题 PDF'}
          </button>
